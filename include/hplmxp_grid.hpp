 ...
 * ---------------------------------------------------------------------
 */
#ifndef HPLMXP_GRID_H
#define HPLMXP_GRID_H
/*
 * ---------------------------------------------------------------------
 * Include files
 * ---------------------------------------------------------------------
 ...
 * ---------------------------------------------------------------------
 */
typedef enum { HPLMXP_INT = 100, HPLMXP_DOUBLE = 101 } HPLMXP_T_TYPE;

typedef enum {
  HPLMXP_ROW_MAJOR    = 201,
  HPLMXP_COLUMN_MAJOR = 202
} HPLMXP_T_ORDER;

typedef struct HPLMXP_S_grid {
  MPI_Comm       all_comm;    /* grid communicator */
  MPI_Comm       row_comm;    /* row communicator */
  MPI_Comm       col_comm;    /* column communicator */
  HPLMXP_T_ORDER order;       /* ordering of the procs in the grid */
 ...
 * ---------------------------------------------------------------------
 */
#define HPLMXP_2_MPI_TYPE(typ) ((typ == HPLMXP_INT ? MPI_INT : MPI_DOUBLE))
/*
 ...
 * ---------------------------------------------------------------------
 */
int HPLMXP_grid_init(MPI_Comm,
                     const HPLMXP_T_ORDER,
                     const int,
                     const int,
                     const int,
                     const int,
...
int HPLMXP_grid_exit(HPLMXP_T_grid&);
int HPLMXP_grid_info(const HPLMXP_T_grid&, int&, int&, int&, int&);
