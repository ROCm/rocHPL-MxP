...
static __launch_bounds__(DIM_X* DIM_Y) __global__
    void gemv_kernel_f8(const int m,
                     const int n,
                     const double   alpha,
  ...
  res_A[0] = res_A[1] = res_A[2] = res_A[3] = double{0};
  ...
  for(col = ty * 4; col < (n - n_tail); col += 4 * DIM_Y) {
    ...
    if(ind < m) {
      res_A[0] += static_cast<double>(float{A[ind + (col + 0) * lda]}) * res_x[0];
      res_A[0] += static_cast<double>(float{A[ind + (col + 1) * lda]}) * res_x[1];
      res_A[0] += static_cast<double>(float{A[ind + (col + 2) * lda]}) * res_x[2];
      res_A[0] += static_cast<double>(float{A[ind + (col + 3) * lda]}) * res_x[3];

      if(ind + DIM_X < m) {
        res_A[1] += static_cast<double>(float{A[ind + DIM_X + (col + 0) * lda]}) * res_x[0];
        res_A[1] += static_cast<double>(float{A[ind + DIM_X + (col + 1) * lda]}) * res_x[1];
        res_A[1] += static_cast<double>(float{A[ind + DIM_X + (col + 2) * lda]}) * res_x[2];
        res_A[1] += static_cast<double>(float{A[ind + DIM_X + (col + 3) * lda]}) * res_x[3];

        if(ind + 2 * DIM_X < m) {
          res_A[2] +=
              static_cast<double>(float{A[ind + 2 * DIM_X + (col + 0) * lda]}) * res_x[0];
          res_A[2] +=
              static_cast<double>(float{A[ind + 2 * DIM_X + (col + 1) * lda]}) * res_x[1];
          res_A[2] +=
              static_cast<double>(float{A[ind + 2 * DIM_X + (col + 2) * lda]}) * res_x[2];
          res_A[2] +=
              static_cast<double>(float{A[ind + 2 * DIM_X + (col + 3) * lda]}) * res_x[3];

          if(ind + 3 * DIM_X < m) {
            res_A[3] +=
                static_cast<double>(float{A[ind + 3 * DIM_X + (col + 0) * lda]}) * res_x[0];
            res_A[3] +=
                static_cast<double>(float{A[ind + 3 * DIM_X + (col + 1) * lda]}) * res_x[1];
            res_A[3] +=
                static_cast<double>(float{A[ind + 3 * DIM_X + (col + 2) * lda]}) * res_x[2];
            res_A[3] +=
                static_cast<double>(float{A[ind + 3 * DIM_X + (col + 3) * lda]}) * res_x[3];
          }
        }
      }
    }
  ...
  // if n is not multiple of (DIM_Y * 4)
  if(n_tail > 0) {
    res_x[0] = res_x[1] = res_x[2] = res_x[3] = double{0};
    ...
    if(ind < m) {
      res_A[0] +=
          static_cast<double>(float{A[ind + (col + 0) * lda * (col + 0 < n)]}) * res_x[0];
      res_A[0] +=
          static_cast<double>(float{A[ind + (col + 1) * lda * (col + 1 < n)]}) * res_x[1];
      res_A[0] +=
          static_cast<double>(float{A[ind + (col + 2) * lda * (col + 2 < n)]}) * res_x[2];
      res_A[0] +=
          static_cast<double>(float{A[ind + (col + 3) * lda * (col + 3 < n)]}) * res_x[3];

      if(ind + DIM_X < m) {
        res_A[1] +=
            static_cast<double>(float{A[ind + DIM_X + (col + 0) * lda * (col + 0 < n)]}) *
            res_x[0];
        res_A[1] +=
            static_cast<double>(float{A[ind + DIM_X + (col + 1) * lda * (col + 1 < n)]}) *
            res_x[1];
        res_A[1] +=
            static_cast<double>(float{A[ind + DIM_X + (col + 2) * lda * (col + 2 < n)]}) *
            res_x[2];
        res_A[1] +=
            static_cast<double>(float{A[ind + DIM_X + (col + 3) * lda * (col + 3 < n)]}) *
            res_x[3];

        if(ind + 2 * DIM_X < m) {
          res_A[2] +=
              static_cast<double>(
                  float{A[ind + 2 * DIM_X + (col + 0) * lda * (col + 0 < n)]}) *
              res_x[0];
          res_A[2] +=
              static_cast<double>(
                  float{A[ind + 2 * DIM_X + (col + 1) * lda * (col + 1 < n)]}) *
              res_x[1];
          res_A[2] +=
              static_cast<double>(
                  float{A[ind + 2 * DIM_X + (col + 2) * lda * (col + 2 < n)]}) *
              res_x[2];
          res_A[2] +=
              static_cast<double>(
                  float{A[ind + 2 * DIM_X + (col + 3) * lda * (col + 3 < n)]}) *
              res_x[3];

          if(ind + 3 * DIM_X < m) {
            res_A[3] +=
                static_cast<double>(
                    float{A[ind + 3 * DIM_X + (col + 0) * lda * (col + 0 < n)]}) *
                res_x[0];
            res_A[3] +=
                static_cast<double>(
                    float{A[ind + 3 * DIM_X + (col + 1) * lda * (col + 1 < n)]}) *
                res_x[1];
            res_A[3] +=
                static_cast<double>(
                    float{A[ind + 3 * DIM_X + (col + 2) * lda * (col + 2 < n)]}) *
                res_x[2];
            res_A[3] +=
                static_cast<double>(
                    float{A[ind + 3 * DIM_X + (col + 3) * lda * (col + 3 < n)]}) *
                res_x[3];
          }
        }
      }
...
void HPLMXP_gemv(const int m,
  ...
  gemv_kernel_f8<GEMVN_DIM_X, GEMVN_DIM_Y>
      <<<gemvn_grid, gemvn_threads, 0, computeStream>>>(
          m, n, alpha, A, lda, x, 1, beta, y, 1);
  HIP_CHECK(hipGetLastError());
}
