...
__launch_bounds__(BLOCK_DIM) __global__
    void HPLMXP_lacpy_kernel(const int M,
                             const int N,
  ...
  if(complete_block) {
    ...
    if(4 * I + 0 < M) Bj[4 * I + 0] = Aj[4 * I + 0];
    if(4 * I + 1 < M) Bj[4 * I + 1] = Aj[4 * I + 1];
    if(4 * I + 2 < M) Bj[4 * I + 2] = Aj[4 * I + 2];
    if(4 * I + 3 < M) Bj[4 * I + 3] = Aj[4 * I + 3];
  }
}
...
// Specialization for converting from __half to float
template <>
__launch_bounds__(BLOCK_DIM) __global__
    void HPLMXP_lacpy_kernel(const int M,
                             const int N,
  ...
  const __half* __restrict__ Aj = A + J * static_cast<size_t>(LDA);
...
__launch_bounds__(BLOCK_DIM) __global__
  ...
  if(complete_block) {
    ...
    if(4 * I + 0 < M) Bj[4 * I + 0] = hipblaslt_f8_fnuz{Aj[4 * I + 0]};
    if(4 * I + 1 < M) Bj[4 * I + 1] = hipblaslt_f8_fnuz{Aj[4 * I + 1]};
    if(4 * I + 2 < M) Bj[4 * I + 2] = hipblaslt_f8_fnuz{Aj[4 * I + 2]};
    if(4 * I + 3 < M) Bj[4 * I + 3] = hipblaslt_f8_fnuz{Aj[4 * I + 3]};
  }
}

// Specialization for converting from hipblaslt_f8_fnuz to float
template <>
__launch_bounds__(BLOCK_DIM) __global__
    void HPLMXP_lacpy_kernel(const int M,
                             const int N,
  ...
  if(complete_block) {
    ...
    if(4 * I + 0 < M) Bj[4 * I + 0] = float{Aj[4 * I + 0]};
    if(4 * I + 1 < M) Bj[4 * I + 1] = float{Aj[4 * I + 1]};
    if(4 * I + 2 < M) Bj[4 * I + 2] = float{Aj[4 * I + 2]};
    if(4 * I + 3 < M) Bj[4 * I + 3] = float{Aj[4 * I + 3]};
  }
}
...
void HPLMXP_lacpy(const int M,
   ...
   * =======
   *
   * HPLMXP_lacpy copies the an array A into an array B.
   *
   *
   * Arguments
   * =========
  ...
  HPLMXP_lacpy_kernel<<<grid_size, block_size, 0, computeStream>>>(
      M, N, A, LDA, B, LDB);
  HIP_CHECK(hipGetLastError());
}

template void HPLMXP_lacpy(const int     m,
                           const int     n,
                           const double* A,
                           const int     lda,
                           double*       B,
