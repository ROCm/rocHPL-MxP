...
__launch_bounds__(BLOCK_DIM) __global__
  ...
  if(4 * I + 0 == J) Aij[0] = T{1.0};
  if(4 * I + 1 == J) Aij[1] = T{1.0};
  if(4 * I + 2 == J) Aij[2] = T{1.0};
  if(4 * I + 3 == J) Aij[3] = T{1.0};
...
template void HPLMXP_identity(const int m, hipblaslt_f8_fnuz* A, const int lda);
