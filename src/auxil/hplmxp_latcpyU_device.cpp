...
__launch_bounds__(BLOCK_DIM_X* BLOCK_DIM_Y) __global__
    void HPLMXP_latcpyU_kernel(const int M,
                               const int N,
  ...
  const bool complete_block =
      (blockIdx.x != blockIdx.y) && (M / (TILE_DIM_X) != blockIdx.y);
...
__launch_bounds__(BLOCK_DIM_X* BLOCK_DIM_Y) __global__
    void HPLMXP_latcpyU_kernel_half(const int M,
                                    const int N,
...
template <int BLOCK_DIM_X, int BLOCK_DIM_Y, int TILE_DIM_X, int TILE_DIM_Y>
__launch_bounds__(BLOCK_DIM_X* BLOCK_DIM_Y) __global__
    void HPLMXP_latcpyU_kernel_fp8(const int M,
                                    const int N,
                                    const float* __restrict__ A,
                                    const int LDA,
...
void HPLMXP_latcpyU(const int M,
   ...
   * =======
   *
   * HPLMXP_latcpyU copies the upper-triangular piece of an array A into the
   * lower-triangular piece of an array B.
   *
   *
   * Arguments
   * =========
...
void HPLMXP_latcpyU(const int    M,
  ...
  HPLMXP_latcpyU_kernel_fp8<BLOCK_DIM_X, BLOCK_DIM_Y, TILE_DIM_X, TILE_DIM_Y>
      <<<grid_size, block_size, 0, computeStream>>>(M, N, A, LDA, B, LDB);
  HIP_CHECK(hipGetLastError());
}
