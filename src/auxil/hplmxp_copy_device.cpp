...
static __launch_bounds__(BLOCK_DIM) __global__
    void hpl_copy_knl(const int N, const T* __restrict__ x, T* __restrict__ y) {
...
template void HPLMXP_copy(const int n, const float* x, float* y);

template void HPLMXP_copy(const int n, const __half* x, __half* y);
