...
__launch_bounds__(BLOCK_DIM) __global__
  ...
  if(complete_block) {
    ...
    if(4 * I + 0 <= J) Bj[4 * I + 0] = Aj[4 * I + 0];
    if(4 * I + 1 <= J) Bj[4 * I + 1] = Aj[4 * I + 1];
    if(4 * I + 2 <= J) Bj[4 * I + 2] = Aj[4 * I + 2];
    if(4 * I + 3 <= J) Bj[4 * I + 3] = Aj[4 * I + 3];
  }
}
...
void HPLMXP_lacpyU(const int M,
   ...
   * =======
   *
   * HPLMXP_lacpyU copies the upper-triangular piece of an array A into an array
   * B.
   *
   *
   * Arguments
