...
static __launch_bounds__(BLOCK_DIM) __global__
    void ptranspose_knl(const int n,
                        const int b,
                        const int myrow,
                        const int mycol,
                        const int nprow,
...
// Transpose a length N column vector distributed
// over an nprow x npcol process grid with
// blocking factor NB, into a row vector
template <typename T>
