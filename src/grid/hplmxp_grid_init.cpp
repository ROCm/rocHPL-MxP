...
int HPLMXP_grid_init(MPI_Comm             COMM,
                     const HPLMXP_T_ORDER ORDER,
                     const int            NPROW,
                     const int            NPCOL,
                     const int            p,
                     const int            q,
   ...
   * =======
   *
   * HPLMXP_grid_init creates a NPROW x NPCOL  process  grid using column- or
   ...
   *         formed.
   *
   * ORDER   (global input)                const HPLMXP_T_ORDER
   *         On entry, ORDER specifies how the processes should be ordered
   *         in the grid as follows:
   *            ORDER = HPLMXP_ROW_MAJOR    row-major    ordering;
   *            ORDER = HPLMXP_COLUMN_MAJOR column-major ordering;
   *
   ...
   *
   * GRID    (local input/output)          HPLMXP_T_grid *
  ...
  if((nprocs > size) || (NPROW < 1) || (NPCOL < 1)) {
    HPLMXP_pabort(__LINE__, "HPLMXP_grid_init", "Illegal Grid");
  }
  /*
  ...
  if(ORDER == HPLMXP_ROW_MAJOR) {
    GRID.order = HPLMXP_ROW_MAJOR;
    ...
    GRID.order = HPLMXP_COLUMN_MAJOR;
