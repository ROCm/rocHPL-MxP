...
int HPLMXP_bcast_blonM(T* SBUF, int SCOUNT, int ROOT, MPI_Comm COMM) {
  ...
  if(rank == ROOT) {
    if(ierr == MPI_SUCCESS)
      ierr = MPI_Send(
          SBUF, SCOUNT, T2MPI<T>::type, MModAdd1(rank, size), tag, COMM);
  } else if(prev == ROOT) {
    if(ierr == MPI_SUCCESS)
      ierr = MPI_Recv(
          SBUF, SCOUNT, T2MPI<T>::type, ROOT, tag, COMM, MPI_STATUS_IGNORE);
  }
  /*
    ...
    if((mydist & mask) == 0) {
      ...
      if((mydist & ip2) != 0) {
        ...
        if(lbuf > 0) {
          if(ierr == MPI_SUCCESS)
            ierr =
                MPI_Send(SBUF + ibuf, lbuf, T2MPI<T>::type, partner, tag, COMM);
        }
      }
    }
  ...
  for(k = 0; k < npm2; k++) {
    ...
    if(lbufR > 0) {
      if(ierr == MPI_SUCCESS)
        ierr = MPI_Irecv(
            SBUF + ibufR, lbufR, T2MPI<T>::type, partner, tag, COMM, &request);
    }

    if(lbufS > 0) {
      if(ierr == MPI_SUCCESS)
        ierr =
            MPI_Send(SBUF + ibufS, lbufS, T2MPI<T>::type, partner, tag, COMM);
    }
...
template int HPLMXP_bcast_blonM(double*  SBUF,
                                int      SCOUNT,
                                int      ROOT,
                                MPI_Comm COMM);

template int HPLMXP_bcast_blonM(float*   SBUF,
                                int      SCOUNT,
                                int      ROOT,
                                MPI_Comm COMM);

template int HPLMXP_bcast_blonM(__half*  SBUF,
                                int      SCOUNT,
                                int      ROOT,
                                MPI_Comm COMM);
