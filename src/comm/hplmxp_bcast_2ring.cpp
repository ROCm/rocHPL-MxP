...
int HPLMXP_bcast_2ring(T* SBUF, int SCOUNT, int ROOT, MPI_Comm COMM) {
  ...
  T* SBUF0 = SBUF;
  T* SBUF1 = SBUF + NchunksHalf * chunk_size;
  ...
  if(Nr0 > 0) {
    MPI_Irecv(RBUF0, Nr0, T2MPI<T>::type, prev, prev, COMM, request + 0);
  }

  /*Recv from right*/
  int Nr1 = std::min(Nrecv1, chunk_size);
  if(Nr1 > 0) {
    MPI_Irecv(RBUF1, Nr1, T2MPI<T>::type, next, next, COMM, request + 1);
  }

  /*Send to right if there is data present to send*/
  int Ns0 = std::min(Nsend0 - Nrecv0, chunk_size);
  if(Ns0 > 0) {
    MPI_Isend(SBUF0, Ns0, T2MPI<T>::type, next, tag, COMM, request + 2);
  }

  /*Send to left if there is data present to send*/
  int Ns1 = std::min(Nsend1 - Nrecv1, chunk_size);
  if(Ns1 > 0) {
    MPI_Isend(SBUF1, Ns1, T2MPI<T>::type, prev, tag, COMM, request + 3);
  }
...
template int HPLMXP_bcast_2ring(double*  SBUF,
                                int      SCOUNT,
                                int      ROOT,
                                MPI_Comm COMM);

template int HPLMXP_bcast_2ring(float*   SBUF,
                                int      SCOUNT,
                                int      ROOT,
                                MPI_Comm COMM);

template int HPLMXP_bcast_2ring(__half*  SBUF,
                                int      SCOUNT,
                                int      ROOT,
                                MPI_Comm COMM);

template int HPLMXP_bcast_2ring(hipblaslt_f8_fnuz*  SBUF,
                                int      SCOUNT,
                                int      ROOT,
                                MPI_Comm COMM);
